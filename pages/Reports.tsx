...
const WorkerDocuments = () => {
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col gap-6 pb-6 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full min-h-0">
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
                  {filteredEmployees.map(emp => (
                      <div key={emp.id} onClick={() => setSelectedEmpId(emp.id)} className={`p-3 rounded-xl cursor-pointer flex items-center gap-3 transition-all ${selectedEmpId === emp.id ? 'bg-blue-600 shadow-md shadow-blue-500/20 transform scale-[1.02]' : 'hover:bg-slate-50 border border-transparent'}`}>
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shadow-inner ${selectedEmpId === emp.id ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'}`}>{emp.status === 'Cancelled' ? 'X' : <User size={18} />}</div>
                          <div className="min-w-0"><div className={`font-bold text-sm truncate ${selectedEmpId === emp.id ? 'text-white' : 'text-slate-800'}`}>{emp.name}</div><div className={`text-xs flex items-center gap-1 ${selectedEmpId === emp.id ? 'text-blue-100' : 'text-slate-400'}`}>{emp.fin}</div></div>
                      </div>
                  ))}
              </div>
          </div>
          <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col overflow-hidden">
              {selectedEmpId ? (
                  <>
                    <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10"><div><h3 className="font-bold text-xl text-slate-800 leading-tight">{employees.find(e => e.id === selectedEmpId)?.name}</h3><p className="text-xs text-slate-500 font-medium">Manage uploaded documents</p></div><div className="flex gap-2"><button onClick={handleDownloadZip} disabled={documents.length === 0 || isZipping} className="flex items-center gap-2 px-5 py-2.5 bg-slate-800 text-white rounded-xl text-sm font-bold hover:bg-slate-700 disabled:opacity-50 transition-all shadow-lg shadow-slate-900/10 active:scale-95">{isZipping ? <span className="animate-spin">⌛</span> : <Package size={18} />}<span className="hidden sm:inline">Download All (ZIP)</span></button></div></div>
                                ...
                                {documents.map(doc => {
                                    ...
                                    return (
                                        <div key={doc.id} className="border border-slate-200 rounded-2xl p-5 hover:shadow-lg hover:border-blue-200 transition-all relative bg-white group flex flex-col justify-between h-full">
                                            <div><div className="flex items-start justify-between mb-3"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600 shadow-sm group-hover:scale-110 transition-transform"><FileText size={24} /></div><div><div className="font-bold text-slate-800 text-sm truncate max-w-[150px]" title={doc.name}>{doc.name}</div><div className="text-xs text-slate-500 font-medium bg-slate-100 px-2 py-0.5 rounded-md inline-block mt-1">{doc.type}</div></div></div><button onClick={() => handleDelete(doc.id)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button></div>{doc.expiryDate && (<div className={`text-[10px] font-bold mt-2 inline-flex items-center px-2 py-0.5 rounded-md ${isExpired ? 'bg-red-100 text-red-600' : isExpiringSoon ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'}`}>{isExpired ? 'EXPIRED' : isExpiringSoon ? 'EXPIRING SOON' : 'VALID'}<span className="mx-1">•</span>{format(parseISO(doc.expiryDate), 'dd MMM yyyy')}</div>)}</div><div className="flex items-center justify-between text-xs mt-4 pt-4 border-t border-slate-50"><span className="text-slate-400 font-medium">{format(parseISO(doc.uploadDate), 'dd MMM')}</span><button onClick={() => handleDownload(doc)} className="text-blue-600 font-bold hover:text-blue-700 hover:underline flex items-center gap-1.5 bg-blue-50 px-3 py-1.5 rounded-lg transition-colors"><Download size={14} /> Download</button></div></div>
                                    );
                                })}
                            </div>
                        )}
