...
import { Plus, Search, Trash2, Edit2, User, FileSpreadsheet, RotateCcw, Ban, Calendar, Loader2, DollarSign, CreditCard } from 'lucide-react';
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
             <div className="flex bg-slate-100 p-1 rounded-xl w-full sm:w-auto">
                 <button onClick={() => setViewMode('active')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${viewMode === 'active' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                     <User size={16} /> Active
                 </button>
                 <button onClick={() => setViewMode('cancelled')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${viewMode === 'cancelled' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                     <Trash2 size={16} /> Cancelled
                 </button>
             </div>
        
            <div className="flex gap-2 w-full sm:w-auto">
                <button onClick={() => exportData('xlsx')} className="flex-1 sm:flex-none p-3 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-100 transition shadow-sm border border-emerald-100 flex items-center justify-center" title="Export Excel"><FileSpreadsheet size={20} /></button>
                <button onClick={() => navigate('/employees/new')} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-xl font-bold transition-all shadow-lg shadow-blue-500/20 active:scale-95"><Plus size={20} /> Add</button>
            </div>
        </div>
      </div>
                            ...
                            <div>
                                ...
                                {emp.status === 'Cancelled' && emp.cancellationDate && (
                                    <div className="text-[10px] text-red-500 font-bold mt-1 flex items-center gap-1 bg-red-50 px-1.5 py-0.5 rounded w-fit">
                                        <Calendar size={10} /> {format(new Date(emp.cancellationDate), 'dd MMM yyyy')}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div key={emp.id} className="p-4 space-y-3 bg-white">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                             <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                 <div className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1"><CreditCard size={10} /> FIN No</div>
                                 <div className="font-mono font-bold text-slate-700 mt-0.5">{emp.fin}</div>
                             </div>
                             <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                 <div className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1"><DollarSign size={10} /> Basic</div>
                                 <div className="font-bold text-slate-700 mt-0.5">${emp.basicSalary} <span className="text-[9px] font-normal">({emp.salaryType})</span></div>
                             </div>
                             <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                 <div className="text-[10px] font-bold text-slate-400 uppercase">Pass Type</div>
                                 <div className="font-bold text-slate-700 mt-0.5">{emp.passType}</div>
                             </div>
                             <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                 <div className="text-[10px] font-bold text-slate-400 uppercase">Expiry</div>
                                 <div className="font-bold text-slate-700 mt-0.5">{emp.workPermitExpiry || '-'}</div>
                             </div>
                        </div>

                        <div className="flex gap-2 mt-2">
                             <button onClick={() => navigate(`/employees/${emp.id}`)} className="flex-1 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors flex items-center justify-center gap-1 text-xs font-bold border border-blue-100"><Edit2 size={14} /> Edit</button>
                             {viewMode === 'active' ? (
                                <button onClick={() => handleCancelWorker(emp)} className="flex-1 py-2 text-red-500 bg-white border border-slate-200 hover:bg-red-50 hover:border-red-100 rounded-lg transition-colors flex items-center justify-center gap-1 text-xs font-bold"><Ban size={14} /> Cancel</button>
                            ) : (
                                <>
                                    <button onClick={() => handleReactivateWorker(emp)} className="flex-1 py-2 text-emerald-500 bg-white border border-slate-200 hover:bg-emerald-50 hover:border-emerald-100 rounded-lg transition-colors flex items-center justify-center gap-1 text-xs font-bold"><RotateCcw size={14} /> Restore</button>
                                </>
                            )}
                        </div>
                    </div>
